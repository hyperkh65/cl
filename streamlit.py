...
def draw_container(container_dim, boxes, container_type):
    ...
    container_cbm = (cx / 100) * (cy / 100) * (cz / 100)
...
def create_pdf(container_type, container_dim, boxes, container_cbm):
    ...
    pdf.drawString(50, 700, f"내부 치수: {container_dim['length']} x {container_dim['width']} x {container_dim['height']} cm")
    ...
    for i, (length, width, height, cartons, per_carton, name) in enumerate(boxes):
        ...
        pdf.drawString(70, y - 20, f"크기: {length} x {width} x {height} cm")
